...
public class  CompetitionTeleOp extends OpMode {
    ...
    boolean IsButtonPushed;             boolean IsOn;
...
//      Controls arm direction
        ...
        if (gamepad2.b && !LSButtonPushedA) {
            liftServoSpeedA=liftServoSpeedA+.9;
            LSOnA = !LSOnA;
        ...
        if (gamepad2.y && !LSButtonPushedB) {
            liftServoSpeedB=liftServoSpeedB+.8;
            LSOnB = !LSOnB;
        ...
        if (liftServoSpeedA>.9) {liftServoSpeedA=.9;}
        if (liftServoSpeedA<-.9) {liftServoSpeedA=-.9;}
        if (liftServoSpeedB>.8) {liftServoSpeedB=.8;}
        if (liftServoSpeedB<-.8) {liftServoSpeedB=-.8;}
        ...
        robot.intakeservo.setPower(gamepad2.left_stick_y);

        //Shooter speed
        robot.shooter.setPower(-shooterSpeed);

        //Intake galore
        robot.liftServoA3.setPower(liftServoSpeedB);
        robot.liftServoA2.setPower(liftServoSpeedA);
        robot.liftServoA1.setPower(liftServoSpeedA);

        robot.liftServoB3.setPower(-liftServoSpeedB);
        robot.liftServoB2.setPower(-liftServoSpeedA);
